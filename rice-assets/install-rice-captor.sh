...
sudo apt-get install -q -y \
 i3-wm xclip mpv libnotify-bin thunderbird abiword
...
# ----- clean up ---------------------------------------------------------------
# saves 392 MB disk space and removes 54 packages
sudo apt-get purge -q -y \
 libreoffice* konversation* k3b* whoopsie* audacious* smplayer* smtube* calibre* okular*
...
# ---- configure nfs -----------------------------------------------------------
sudo apt -q -y install nfs-common
mkdir TERA
cat << EOF > bin/tera
#!/bin/bash
sudo mount -o v3 10.0.1.1:/home/rynnon/TERA /home/rynnon/TERA
EOF

# ---- crypto setup -----------------------------------------------------------
sudo apt -q -y install gocryptfs gnome-keyring seahorse enigmail
# copy keyring to .local
# gocryptfs init
# mount gocryptfs vault


# ---- configure programming env ----------------------------------------------
sudo apt -q -y install lxd python3-pip
pip3 install virtualenv
mkdir ~/dev && cd ~/dev
# lxd init
# lxd container starten (rynnonblog)
# git clone automailer
